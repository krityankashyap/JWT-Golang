...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		err := utils.CheckUserType("ADMIN", c) // Only admin can check the details of all users
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		...
		recordsPerPage, err := strconv.Atoi(c.Query("recordsPerPage")) // c.Query returns the query parameter value as a string and strconv.Atoi converts the string to an integer

		if err != nil || recordsPerPage < 1 {
			recordsPerPage = 10 // Default value for recordsPerPage if the query parameter is not provided or is invalid
		}
		page, err1 := strconv.Atoi(c.Query("page"))
		if err1 != nil || page < 1 {
			page = 1 // Default value for page if the query parameter is not provided or is invalid
		}

		startIndex := (page - 1) * recordsPerPage             // Calculate the starting index for pagination
		startIndex, err = strconv.Atoi(c.Query("startIndex")) // c.Query returns the query parameter value as a string

		matchStage := bson.D{{Key: "$match", Value: bson.D{}}} // MatchStage is used to filter the documents based on the provided filter in the query parameter

		groupStage := bson.D{{Key: "$group", Value: bson.D{{Key: "_id", Value: nil}, {Key: "total_count", Value: bson.D{{Key: "$sum", Value: 1}}}, {Key: "data", Value: bson.D{{Key: "$push", Value: "$$ROOT"}}}}}} // GroupStage is used to group the documents based on the provided group in the query parameter

		projectStage := bson.D{{Key: "$project", Value: bson.D{{Key: "_id", Value: 0}, {Key: "total_count", Value: 1}, {Key: "user_items", Value: bson.D{{Key: "$slice", Value: []interface{}{"$data", startIndex, recordsPerPage}}}}}}} // ProjectStage is used to project the required fields in the response based on the provided projection in the query parameter
